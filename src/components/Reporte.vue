      ...
      <div class="col-md-12">
        <b-card no-body>
          <b-tabs pills card vertical>
            ...
            <b-tab
              :title="resultado.nombre_alumno"
              v-for="resultado in resultados"
              :key="resultado.id"
              @click="verInfo(resultado.id)"
            >
              <div class="row">
                <div class="col-md-12 text-center">
                  <h4>
                    {{ resultado.nombre_alumno }} ({{ resultado.alumno_id }})
                  </h4>
                </div>
                <div class="col-md-12 text-center">
...
export default {
  ...
  firestore: {
    resultados: db.collection('comipems').orderBy('nombre_alumno'),
    materias_comipems: db
      .collection('materias_comipems')
  ...
  methods: {
        ...
        .then((querySnapshot) => {
          ...
          for (const doc of querySnapshot.docs) {
            puntajeMateriaPromesas.push(
              doc.ref.collection('puntaje_por_materia').get(),
            );
          }
    ...
    procesarPromesas(id, promesas) {
      ...
      Promise.allSettled([...promesas, examenesRef]).then((results) => {
        results;
    ...
    procesarPuntajes(id, puntajes, nombre_examenes) {
      ...
      for (const puntaje of puntajes) {
        puntaje.sort((a, b) => {
          return a.materia.toLowerCase().localeCompare(b.materia.toLowerCase());
        });
      }
      ...
      for (const puntaje of puntajes) {
        ...
        for (const mc_nombre of materias_comipems_nombre) {
          if (this.objectContainsSubjectInArray(puntaje, mc_nombre)) {
            puntajes_i.push(
              this.getScoreFromSubjectInArray(puntaje, mc_nombre),
            );
          } else {
            puntajes_i.push(0);
          }
      ...
      this.cargarChartPorMateria(
        id,
        materias_comipems_nombre,
        materias,
        nombre_examenes,
      );
    },
